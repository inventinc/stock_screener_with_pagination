/**
 * Heroku-compatible Financial Modeling Prep API Import Script
 * Imports all stocks with all data points without filesystem dependencies
 */
...
const RATE_CONFIG = {
  initialConcurrency: 3,        // Start with conservative concurrency for Heroku
  minConcurrency: 1,            // Minimum concurrency
  maxConcurrency: 10,           // Maximum concurrency for Heroku
  concurrencyStep: 1,           // How much to adjust concurrency
  initialBackoff: 500,          // Initial backoff in ms
  maxBackoff: 10000,            // Maximum backoff in ms
  ...
  requestSpacing: 200           // Minimum ms between requests for Heroku
};
...
console.log('Starting Heroku-compatible FMP import process...');
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/stocksDB')
  .then(() => {
    console.log('Connected to MongoDB');
    return importAllStocksAdaptive();
  })
  .then((result) => {
