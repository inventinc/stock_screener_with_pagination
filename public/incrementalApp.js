/**
 * Incremental app.js with optimized batch loading for 5,700+ stocks
...
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM fully loaded - initializing incremental app with optimized batch loading");
    ...
    // Add load more button and loading mode toggle
    loadMoreBtn = document.getElementById('load-more-btn') || createLoadMoreButton();
    loadingModeToggle = document.getElementById('loading-mode-toggle') || createLoadingModeToggle();
...
function createLoadMoreButton() {
    ...
    stockCardsContainer.parentNode.insertBefore(container, stockCardsContainer.nextSibling);
    return button;
}

/**
 * Create loading mode toggle if it doesn't exist
 * @returns {HTMLElement} The loading mode toggle element
 */
function createLoadingModeToggle() {
    const container = document.createElement('div');
    container.id = 'loading-mode-container';
    container.className = 'loading-mode-container';
    container.style.display = 'flex';
    ...
    label.className = 'loading-mode-label';
    label.htmlFor = 'loading-mode-toggle';
    label.textContent = 'Manual Loading';
    ...
    toggle.id = 'loading-mode-toggle';
    toggle.className = 'loading-mode-toggle';
    
    container.appendChild(label);
    container.appendChild(toggle);
    
    // Insert before view controls
    const viewControls = document.querySelector('.view-controls');
    if (viewControls) {
        viewControls.parentNode.insertBefore(container, viewControls);
    }
...
function setupEventListeners() {
    ...
    // Loading mode toggle
    if (loadingModeToggle) {
        loadingModeToggle.addEventListener('change', function() {
            const manualMode = this.checked;
            setLoadingMode(manualMode);
        });
    }
...
function initializeDataManager() {
    ...
    stockDataManager = new IncrementalDataManager({
        ...
        manualLoadMode: loadingModeToggle ? loadingModeToggle.checked : false
    });
...
function updateLoadMoreButton(hasMoreData) {
    if (loadMoreBtn) {
        const manualMode = loadingModeToggle ? loadingModeToggle.checked : false;
        loadMoreBtn.style.display = (manualMode && hasMoreData) ? 'block' : 'none';
    }
}

/**
 * Set loading mode
 * @param {Boolean} manualMode - Whether to use manual loading mode
 */
function setLoadingMode(manualMode) {
    console.log(`Setting loading mode to ${manualMode ? 'manual' : 'automatic'}`);
    
    // Update data manager
    if (stockDataManager) {
        stockDataManager.setManualLoadMode(manualMode);
    }
    
    // Update virtual scroller
    if (virtualScroller) {
        virtualScroller.setManualLoadMode(manualMode);
    }
    
    // Update load more button
    if (stockDataManager) {
        const status = stockDataManager.getLoadingStatus();
        updateLoadMoreButton(status.hasMoreData);
    }
}
