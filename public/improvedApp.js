/**
 * Improved incremental app.js with optimized batch loading for 5,700+ stocks
 * Implements smaller batch sizes (50-75) for better user experience
 * Fixes UI issues and implements localStorage for persistence
 */
...
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM fully loaded - initializing improved incremental app");
...
function createLoadMoreButton() {
    ...
    // Ensure it's appended to the correct location
    const stocksSection = document.querySelector('.stocks-section');
    if (stocksSection) {
        stocksSection.appendChild(container);
    } else {
...
function setupEventListeners() {
    ...
    // Add scroll event listener to container for automatic loading
...
/**
 * Check if we should automatically load more stocks based on scroll position
 */
function checkForAutoLoad() {
    if (!stockCardsContainer || !stockDataManager || !loadMoreBtn) return;
    ...
    if (totalHeight - (scrollPosition + containerHeight) < 300) {
        ...
        if (status.hasMoreData && !status.isLoading && loadMoreBtn.style.display !== 'none') {
            loadMoreStocks();
        }
    }
}
...
function initializeDataManager() {
    ...
    if (cachedData && cachedData.stocks && cachedData.stocks.length > 0) {
        ...
        if (cachedData.stats) {
            updateHeaderStatsFromCache(cachedData.stats);
        }
...
function updateLoadingProgress(loaded, total, fromCache = false) {
    ...
    if (loadingProgressText) {
        loadingProgressText.textContent = fromCache 
            ? `Loaded ${loaded} of ${total} stocks from cache (${progress.toFixed(0)}%)`
    ...
    updateLoadMoreButton(loaded < total);
}
 ...
 */
function updateLoadMoreButton(hasMoreData) {
    if (loadMoreBtn) {
 ...
 * @param {Object} stats - Stats object from cache
 */
function updateHeaderStatsFromCache(stats) {
    if (totalStocksEl) totalStocksEl.textContent = stats.total || 0;
    if (nyseStocksEl) nyseStocksEl.textContent = stats.nyse || 0;
    if (nasdaqStocksEl) nasdaqStocksEl.textContent = stats.nasdaq || 0;
    if (lastUpdatedEl) lastUpdatedEl.textContent = stats.lastUpdated || new Date().toLocaleTimeString();
}
...
function updateTableView(stocks) {
    ...
    // Add rows for each stock (limit to first 100 for performance)
    ...
    displayStocks.forEach(stock => {
        ...
        row.innerHTML = `
            ...
            <td>$${(stock.price || (stock.last_trade && stock.last_trade.p) || 0).toFixed(2)}</td>
            <td>${formatMarketCap(stock.market_cap || stock.marketCap)}</td>
            <td>${(stock.pe_ratio || stock.peRatio || 0).toFixed(2)}</td>
            <td>${((stock.dividend_yield || stock.dividendYield || 0) * 100).toFixed(2)}%</td>
            <td><span class="score ${getScoreClass(stock.custom_score || stock.score || 0)}">${(stock.custom_score || stock.score || 0).toFixed(2)}</span></td>
        `;
        
        stockTableBody.appendChild(row);
    });
    
    // Add a "load more" row if there are more stocks
    if (stocks.length > 100) {
        const loadMoreRow = document.createElement('tr');
        loadMoreRow.innerHTML = `
            <td colspan="8" style="text-align: center; padding: 15px;">
                <button class="table-load-more-btn">Load More Rows</button>
            </td>
        `;
        
        const loadMoreBtn = loadMoreRow.querySelector('.table-load-more-btn');
        if (loadMoreBtn) {
            loadMoreBtn.addEventListener('click', function() {
                // Load more rows in the table
                updateTableView(stocks);
            });
        }
        
        stockTableBody.appendChild(loadMoreRow);
    }
}
...
function saveToLocalStorage(stocks) {
    try {
        // Get current stats
        const stats = {
            total: totalStocksEl ? parseInt(totalStocksEl.textContent) : 0,
            nyse: nyseStocksEl ? parseInt(nyseStocksEl.textContent) : 0,
            nasdaq: nasdaqStocksEl ? parseInt(nasdaqStocksEl.textContent) : 0,
            lastUpdated: lastUpdatedEl ? lastUpdatedEl.textContent : new Date().toLocaleTimeString()
        };
