/**
 * Force Import All Stocks Script
 * 
 * This script forces a full import of all stocks from Financial Modeling Prep API
 * regardless of existing database content, and collects comprehensive data points.
 * 
 * Usage:
 * - Manual: node forceImportAllStocks.js
 * - Heroku: heroku run node forceImportAllStocks.js --app your-app-name
 */
...
// Log file path
const logFilePath = path.join(logsDir, 'force_import_log.txt');
...
// Premium plan optimized settings - adjusted to avoid rate limits
const CONCURRENT_REQUESTS = 15;  // Balanced concurrency
const BATCH_SIZE = 50;           // Batch size
const REQUEST_DELAY = 150;       // Delay between requests
const MAX_RETRIES = 3;           // Maximum number of retries for rate-limited requests
const RETRY_DELAY = 2000;        // Base delay before retry (will increase exponentially)
...
/**
 * Make API request to Financial Modeling Prep with retry logic
...
async function makeApiRequest(endpoint, params = {}, retries = MAX_RETRIES) {
  try {
    ...
    if (error.response && error.response.status === 429 && retries > 0) {
      ...
      log(`Rate limited, retrying after ${delay}ms (${retries} retries left)`);
...
/**
 * Process a batch of stocks with rate limit awareness
  ...
  for (const stock of stockBatch) {
    ...
    const promise = (async () => {
      try {
        ...
        const stockData = await getComprehensiveStockData(stock.symbol);
...
/**
 * Force import all stocks from FMP
 * @param {Object} Stock - Mongoose Stock model
 * @returns {Promise<void>}
 */
async function forceImportAllStocks(Stock) {
  try {
    log('Starting FORCED FULL IMPORT of all stocks from Financial Modeling Prep...');
    ...
    log(`Force import complete. Total stocks imported: ${totalImported}`);
  } catch (error) {
...
/**
 * Main function to force import all stocks
 * @returns {Promise<boolean>} - Success status
 */
async function main() {
  try {
    log('Starting forced full import of all stocks with comprehensive data...');
    ...
    log(`Current database has ${initialCount} stocks before force import`);
    
    // Force import all stocks
    await forceImportAllStocks(Stock);
    
    // Get new stock count
    const finalCount = await Stock.countDocuments();
    log(`Database now has ${finalCount} stocks after force import`);
    log(`Added ${finalCount - initialCount} new stocks to the database`);
    
    // Calculate duration
    const duration = ((Date.now() - startTime) / 1000 / 60).toFixed(2);
    
    log(`Force import completed successfully in ${duration} minutes`);
    ...
    log(`Error during force import: ${error.message}`);
