/**
 * Optimized Premium Stock Data Refresh Script for Heroku
 * 
 * Enhanced version for Financial Modeling Prep Premium API users
 * with rate limit handling and comprehensive data collection
 * 
 * Usage:
 * - Manual: node optimizedPremiumRefresh.js
 * - Heroku: heroku run node optimizedPremiumRefresh.js --app your-app-name
 */
...
const CONCURRENT_REQUESTS = 15;  // Reduced from 30 to 15
const BATCH_SIZE = 50;           // Keep batch size at 50
const REQUEST_DELAY = 150;       // Increased from 50ms to 150ms
 ...
 * @returns {Promise<void>}
 */
async function refreshStocks(Stock, limit = 200) {
  try {
...
async function autoRefreshStocks() {
  try {
    log('Starting optimized premium stock data refresh...');
